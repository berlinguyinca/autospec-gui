...
const source = readFileSync(pagePath, "utf8")
  .replace(/^import\s+\{ AutospecConfigError \}[^;]+;\s*$/m, "class AutospecConfigError extends Error {}")
  .replace(/^import\s+\{ getTelemetryOverview, type AutonomousRunStatus \}[^;]+;\s*$/m, "const getTelemetryOverview = async () => ({ autonomousRunStatus: null });")
